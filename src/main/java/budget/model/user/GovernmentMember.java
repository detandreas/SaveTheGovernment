...
public class GovernmentMember extends User {
    ...
    /**
     * Creates a new GovernmentMember with the specified details.
     ...
     * @param ministry GovernmentMember ministry it belongs to.
     */
    public GovernmentMember(String username, String fullName, String password,
            Ministry ministry) {
    ...
    /**
     * Returns the ministry of this government member.
     ...
     * @param ministry The ministry to assign to this member.
     */
     ...
     * @return {@code false} as government members cannot approve changes.
     */
    @Override
     ...
     */
    public PendingChange submitChangeRequest(BudgetItem item,
            double newAmount) {
        int userId = 0; /* placeholder for user id */
        return new PendingChange(item.getId(), getUserName(),
                userId, item.getValue(), newAmount);
    }
}
